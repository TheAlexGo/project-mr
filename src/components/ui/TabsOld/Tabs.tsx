...
import { Content } from '@components/TabsOld/components/Content/Content';
import { ITab, Tab } from '@components/TabsOld/components/Tab/Tab';
...
export const Tabs: FC<ITabs> = ({
    ...
    const keyDownHandler = useCallback(
        (e: KeyboardEvent) => {
            if (e.key === KeyboardKeys.ARROW_RIGHT || e.key === KeyboardKeys.ARROW_LEFT) {
                const { current } = tabElements;
                let newValue = focusedTab;
                if (e.key === KeyboardKeys.ARROW_RIGHT) {
                    newValue += 1;
